...
- Per project memory, **prod deploys are currently on hold** (DevExpress licensing issue affecting the shared `impactdisciples-admin` dependency) — do not merge to `master` or run `build-deploy-prod` without the user's explicit go-ahead, even if a change looks self-contained to this repo.
...
- Firestore rules (`firestore.rules`) are currently wide open (`allow read, write: if true`) — a known, tracked issue, not something to silently "fix" as a drive-by change since tightening it can break existing client code paths that assume open access.
