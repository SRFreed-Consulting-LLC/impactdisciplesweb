...
export class CheckoutComponent implements OnInit, OnDestroy {
  ...
  calculateShippingCost = () => {
    ...
    this.webConfigService.getAll().then(configs => {
      if(configs){
        this.shippingService.calculateShipping(configs[0], this.checkoutForm.firstName + ' ' + this.checkoutForm.lastName, this.checkoutForm.shippingAddress, this.checkoutForm.phone, totalWeight)
        .then(result => {
          if(result) {
